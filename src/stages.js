...
export const STAGES = [
  {
    ...
    // ── ด่านนี้ไม่มีอันตรายที่ขยับได้แล้ว ──
    // เดิมมีไฟเตาอบสลับพ่นจากพื้นกับจากเพดาน ถอดออกไปแล้ว ความยากของด่าน
    // จึงมาจากผังท่อนล้วน ๆ เหมือนด่านที่ยังไม่มี hazard ตัวอื่น
    //
    // คลังยังเป็นชุดท่อนโล่ง ๆ ตามเดิม ถ้าวันหลังจะใส่อันตรายกลับมา
    // ให้เติมช่อง hazard แล้วคลังชุดนี้จะรับได้ทันทีโดยไม่ต้องแก้อะไรอีก
    // ── ลำดับท่อนที่จัดเองทั้งด่าน ──
    // ทำจากหน้าออกแบบด่าน (editor.html โหมดทั้งด่าน) แล้วส่งออกมา
    // ไม่มี pool/segments แล้ว เพราะ Level.routeFor อ่าน pool ก่อนเสมอ
    // ถ้าใส่ pool กลับมา route ชุดนี้จะถูกมองข้ามทั้งก้อน
    route: [
      { p: 51 },
      { p: 52, kibble: 'cluster' },
      { p: 33, shrimp: true },
    ...
    gate: 'kitchen',
    // ฉากหลังเป็นครัวจริง ๆ ทั้งห้อง (render/bakery.js) — ผนังกระเบื้อง ชั้นวางขวดโหล
    // เตาอบที่ไฟยังติด หน้าต่างโค้งเห็นพระจันทร์ และเคาน์เตอร์เตรียมของ
    // จานสี sky เดิมกลายเป็น "ท้องฟ้าที่เห็นผ่านหน้าต่าง" ค่าสีทุกตัวจึงยังมีความหมายเหมือนเดิม
    backdrop: 'bakery',
    // ชั้นเดิม (ชั้นวาง/เตา/ท่อ/สะเก็ดไฟ) ถูกถอดออก — ฉากหลังวาดให้ครบทั้งชุดแล้ว
    layers: [],
  },

  {
    ...
    // ไม่มี pool/segments แล้ว เพราะ Level.routeFor อ่าน pool ก่อนเสมอ
    // ถ้าใส่ pool กลับมา route ชุดนี้จะถูกมองข้ามทั้งก้อน
    // และเพราะเป็น route เขียนมือ ด่านนี้จึง "จบจริง" ที่ท่อนสุดท้ายแล้วต่อถ้ำคริสตัลเลย
    // (ไม่วนซ้ำท่อนเดิม — ดู Level.loops / Game.updateScene)
    route: [
      { p: 54 },
      { p: 55 },
      { p: 56, kibble: 'alternate' },
      { p: 57 },
      { p: 58 },
      { p: 59 },
      { p: 60 },
      ...
      { p: 70 },
      { p: 71 },
      { p: 72, kibble: 'alternate' },
      { p: 73 },
    ],
    // เข้าสวนจากฉากก่อนหน้า (ครัวกลางคืน) = มุดผ่านพุ่มดอกไม้ยักษ์ (gates.js)
    gate: 'garden',
    // ── ผึ้งมาจากท่อน ไม่ใช่จากตัวจับเวลา ──
    // เดิมประกาศ hazard ไว้ตรงนี้ แล้วระบบปล่อยผึ้งทุก 215 เฟรมที่ขอบจอขวา
    // ซึ่งแปลว่าตำแหน่งผึ้งไม่เคยตรงกับของที่คนออกแบบวางไว้ในท่อนเลย
    // ตอนนี้ด่านนี้จัดลำดับท่อนเองทั้งด่านแล้ว ผึ้งจึงถูกวางไว้ในท่อน (ดู hazards ใน PATTERNS)
    // ได้จังหวะที่ตั้งใจจริง ๆ — ถ้าอยากได้ผึ้งเพิ่มให้ไปวางในท่อนจากหน้าออกแบบด่าน
    // ฉากหลังใช้ชุดเดียวกับทางเข้าเป๊ะ ๆ (render/meadow.js) — มุดพ้นพุ่มดอกไม้แล้ว
    // เนิน ต้นไม้ ทุ่งดอกไม้ เรือนกระจก และกังหันลมเป็นผืนเดิมต่อไป ไม่มีรอยต่อ
    backdrop: 'meadow',
    // เมฆ/ต้นไม้/กลีบดอกถูกถอดออกแล้ว — ฉากหลังวาดให้ครบทั้งชุด
    // ถ้าปล่อยไว้จะได้เมฆสองชุดกับต้นไม้สองชุดซ้อนกันคนละสเกล
    layers: [],
  },

  {
    ...
    gate: 'cavern',         // ทางเข้า: ภูเขาคริสตัล → อุโมงค์ → โถงถ้ำ (ดู gates.js)
    ...
    // ฉากหลังใช้ชุดเดียวกับทางเข้าเป๊ะ ๆ (render/cave.js) — ออกจากอุโมงค์แล้ว
    // ยอดคริสตัล สายแร่ เพดานหินย้อย และบ่อน้ำเรืองแสงเป็นผืนเดิมต่อไป ไม่มีรอยต่อ
    backdrop: 'cave',
    // ชั้นเพดาน/สายแร่/หมอกถูกถอดออกแล้ว — ฉากหลังวาดให้ครบทั้งชุด
    // เหลือไว้แต่เสาหินชั้นใกล้ซึ่งทำหน้าที่เป็นกรอบข้างของถ้ำ (อยู่หน้าฉากหลัง)
    layers: [
...

  {
    ...
    gate: 'beach',          // ทางเข้า: ซุ้มไม้ริมทะเล → ทางเดินไม้ → พระอาทิตย์ตก (ดู gates.js)
    // ฉากหลังใช้ชุดเดียวกับทางเข้าเป๊ะ ๆ (render/sea.js) — วิ่งออกจากทางเดินไม้แล้ว
    // ทะเล ฟ้า และดวงอาทิตย์เป็นผืนเดิมต่อไป ไม่มีรอยต่อให้เห็นเลย
    backdrop: 'sea',
    ...
    // ดวงอาทิตย์กับคลื่นถูกถอดออกจากชั้นของประกอบแล้ว — ฉากหลัง 'sea' วาดให้ครบทั้งคู่
    // ถ้าปล่อยไว้จะได้ดวงอาทิตย์สองดวงกับคลื่นสองชุดซ้อนกัน เหลือไว้แต่ร่มชายหาดบนทราย
    layers: [
...

  {
    ...
    gate: 'space',          // ทางเข้า: ฐานปล่อยริมทะเล → ทะยานขึ้น → ทะลุเมฆ → โลก → ประตูมิติ
    // ฉากหลังใช้ชุดเดียวกับทางเข้าเป๊ะ ๆ (render/space.js) — ออกจากประตูมิติแล้ว
    // ดาว เนบิวลา และดาวเคราะห์เป็นผืนเดิมต่อไป ไม่มีรอยต่อให้เห็น
    backdrop: 'space',
    ...
    faller: { every: 165 },
    // ดาวเคราะห์ถูกถอดออกจากชั้นของประกอบแล้ว — ฉากหลัง 'space' วาดให้ครบทั้งดาวเคราะห์
    // เนบิวลา และสนามดาว ถ้าปล่อยไว้จะได้ดาวเคราะห์สองชุดซ้อนกัน เหลือไว้แต่สถานีกับฝุ่นดาว
    layers: [
...

  {
    ...
    palette: {
      ...
      // ── พื้นของฉากนี้ไม่ได้มาจากจานสี ──
      // ทุ่งหิมะวาดพื้นด้วยผืนหิมะจริง (GROUND_ART.snow → drawSnowDeck) ซึ่งทับสามชั้นนี้
      // ทั้งหมด เหตุผลคือแผ่นสีเดียวเล่า "หิมะ" ไม่ได้ ของเดิมเป็นน้ำเงินทึบซึ่งอ่านเป็นน้ำ
      // และที่แย่กว่านั้นคือมันไม่เหมือนพื้นของทางเข้าพายุหิมะ ผู้เล่นจึงเห็นพื้นเปลี่ยน
      // กลางคันทุกครั้งที่เข้าด่าน
      //
      // สามค่านี้ยังต้องตั้งให้ถูกไว้ เพราะเป็นสิ่งที่เห็นหนึ่งเฟรมก่อนผืนหิมะจะถูกวาดทับ
      // และเป็นตัวสำรองถ้าวันหนึ่งฉากนี้ถูกใช้โดยไม่มีลายพื้น
      ground: '#AFC9DC', crust: '#DCEAF4', crustTop: '#FFFFFF',
      crumb: 'rgba(255,255,255,0)',
      ...
      // ลูกบอลของแมพนี้คือก้อนหิมะกลิ้ง — drawBall อ่านชุดสีนี้แทนสีชายหาด
      // เสี้ยวต้องต่างค่าความสว่างกันจริง ๆ ไม่งั้นก้อนขาวบนหิมะขาวจะดูเหมือนลอยนิ่ง
      // และผู้เล่นจะอ่านไม่ออกว่ามันกำลังกลิ้งเข้าหา
      ballCols: ['#FFFFFF', '#BCD9EC', '#F2FAFF', '#93B9D4'],
    },
    gate: 'snow',           // ทางเข้า: ร่อนลงจากอวกาศ → พายุหิมะ → ทุ่งหิมะเปิดออก
    // ฉากหลังใช้ชุดเดียวกับทางเข้าเป๊ะ ๆ (render/snow.js) — พายุสงบแล้วทุ่ง เทือกเขา
    // ป่าสน ปราสาท และแสงเหนือเป็นผืนเดิมต่อไป ไม่มีรอยต่อ
    backdrop: 'snow',
    ...
    pool: [0, 26, 27, 28, 29, 30, 4, 18, 2, 9, 12, 26],
    segments: 20,
    ...
    faller: { every: 240 },
    // ก้อนหิมะกลิ้งสวนมา — ระบบเดียวกับลูกบอลชายหาด ต่างที่สีและจังหวะ
    // มาไกล ๆ ให้เห็นก่อนเสมอ (กลิ้งเข้าหาจากขอบจอขวา) จึงอ่านทันแม้ช่วงหิมะตกหนัก
    hazard: { kind: 'ball', every: 270 },
    // ไม่มีชั้นของประกอบแล้ว — ฉากหลัง 'snow' วาดให้ครบทั้งเทือกเขา ป่าสน ผลึก ทุ่ง และหิมะตก
    // ถ้าปล่อยชั้นเดิมไว้จะได้ภูเขาสองชุดกับหิมะสองระบบซ้อนกัน
    layers: [],
  },
];
