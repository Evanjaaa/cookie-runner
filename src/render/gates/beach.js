...
import { drawProps } from '../background.js';
...
function drawInside(ctx, v, alpha) {
  ...
  // ── ฉากหลังวาดเฉพาะเหนือระดับพื้น ──
  // ฉากทะเลมีแถบทรายของตัวเองลงไปถึงขอบล่างจอ และชั้นทางเข้าวาด "ทับ" พื้นของเกม
  // ถ้าไม่ตัด พอพ้นทางเดินไม้ออกมา จะเห็นทรายเรียบของฉากหลังแทนพื้นชายหาดจริง
  // จนทางเข้าเลิกวาด (1-2 วิ) แล้วพื้นจริงถึงโผล่แว้บขึ้นมา — ตัดไว้ที่ GROUND_Y
  // พื้นใต้ทางเดินไม้ถูกชั้นทางเดินปิดอยู่แล้ว ส่วนหลังทางเดินจึงเป็นพื้นด่านจริงตั้งแต่ก้าวแรก
  ctx.save();
  ctx.beginPath();
  ctx.rect(-W, -H, W * 3, H + GROUND_Y);
  ctx.clip();
  drawSeaBackdrop(ctx, cam, tick, { u: v.inside, palms: true });
  ctx.restore();

  // ── ร่มชายหาดของด่าน ──
  // ร่มเป็นชั้นของประกอบของด่าน (layers ใน stages.js) ที่ด่านวาดไว้ "ใต้" ทางเข้า
  // ฉากหลังทะเลข้างบนทึบจึงกลบมันหมด แล้วร่มโผล่พรวดตอนทางเข้าเลิกวาด — วาดซ้ำที่นี่ด้วยข้อมูลชุดเดียวกัน
  // ร่มวางตามพิกัดโลก ตำแหน่งจึงตรงกับของด่านเป๊ะ พอส่งต่อแล้วไม่ขยับเลย (ท่าเดียวกับต้นมะพร้าว)
  const beach = stageById('beach');
  if (beach) drawProps(ctx, cam, beach.layers, 'far', beach.palette, tick);
