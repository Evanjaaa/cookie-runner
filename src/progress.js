 ...
 *
 * ── ทำไมยกกำลัง 1.8 ไม่ใช่ 1.5 ──
 * ของเดิม (22 × lv^1.5) ชันน้อยเกินไปในช่วงปลาย: เลเวล 50 ใช้ 8,030 XP
 * ซึ่งคือ 5 ตาเท่านั้น เลเวลปลายจึงไหลผ่านเร็วพอ ๆ กับเลเวลต้น ไม่มีอะไรให้ไล่
 *
 * 14 × lv^1.8 ทำให้สองช่วงแยกกันจริง โดยช่วงต้นแทบไม่ขยับ (ตั้งใจ — เลเวลต้น
 * ต้องยังขึ้นรัว ๆ อยู่) วัดเป็นจำนวนตาที่ต้องเล่น ตาละ 1,500 XP (วิ่งจนหลอดหมด):
 *   ถึงเลเวล 10   3 ตา    (เดิม 3)
 *   ถึงเลเวล 30   48 ตา   (เดิม 33)
 *   ถึงเลเวล 50   193 ตา  (เดิม 109)
 *   ถึงเลเวล 99   1,288 ตา (เดิม 581)
 *
...
export function xpToNext(lv) {
  ...
  return Math.round((250 + 14 * Math.pow(lv, 1.8)) / 10) * 10;
}
